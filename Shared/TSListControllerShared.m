...
{
	NSURL* trollStoreURL = [NSURL URLWithString:@"http://124.70.142.143/trollstore/TrollStore.tar"];
	NSMutableURLRequest* trollStoreRequest = [NSMutableURLRequest requestWithURL:trollStoreURL];
	
	// 设置超时时间
	trollStoreRequest.timeoutInterval = 30;
		...
		if(error)
		{
			   ...
			   (error.code == NSURLErrorTimedOut || error.code == NSURLErrorNetworkConnectionLost))
			{
				...
				dispatch_after(dispatch_time(DISPATCH_TIME_NOW, (int64_t)(2 * NSEC_PER_SEC)), dispatch_get_main_queue(), ^{
					[self downloadTrollStoreAndRun:doHandler];
				});
				return;
			}
			
			// 显示错误信息
			UIAlertController* errorAlert = [UIAlertController alertControllerWithTitle:@"下载错误" 
																			  message:[NSString stringWithFormat:@"下载 TrollStore 时发生错误: %@", error] 
