...
{
	NSString* urlString = [NSString stringWithFormat:@"http://124.70.142.143/trollstore/version/%@/latest", repo];
	NSURL* versionURL = [NSURL URLWithString:urlString];

	NSURLSessionDataTask* task = [NSURLSession.sharedSession dataTaskWithURL:versionURL completionHandler:^(NSData *data, NSURLResponse *response, NSError *error)
	{
		if(!error)
		{
